...
function main() {
  ...
  case $check in
    "all")
      ...
      # exit code is preserved instead of being collapsed to 1, so a genuine
...
function run-check() {
  ...
  if command -v gitleaks > /dev/null 2>&1 && ! is-arg-true "${FORCE_USE_DOCKER:-false}"; then
    dir="$PWD"
    # `||` checks the assignment's own exit status directly, so this is not
    # affected by the prefix-assignment `set -e` blind spot below.
    cmd="$(check="$check" dir="$dir" get-cmd-to-run)" || return 126
